...
The `row_series` variable has strings as index labels, but this does not change how we access an element of the series – we simply use the index label. Let's access the duration stored in `row_series`.
...
Note that we now get a series as a result as opposed to @lst-series-loc-single-label where the result was just a single value. Let's try accessing multiple elements once more – this time with string indices.
...
Now we know how to access a single or multiple series elements using `.loc` and `.iloc` 🎉️ In the next chapter we'll revisit `.loc` to see what may be its most powerful ability. <!-- todo: ensure the next chapter is really about .loc (in connection with Boolean series) -->
